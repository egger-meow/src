...
__global__ void convolve_vertical(int width, int height, float* input, float* output, int kernel_size, float* kernel) {
    const int thisX = threadIdx.x + blockIdx.x * blockDim.x;
    const int thisY = threadIdx.y + blockIdx.y * blockDim.y;

    if (thisX >= width || thisY >= height) {
        return;
    }

    int center = kernel_size / 2, dy, tx, ty;
    ...
    for (int k = 0; k < kernel_size; k++) {
        ...
        sum += input[ty * width + tx] * kernel[k];
    }

    output[thisY * width + thisX] = sum;
}

__global__ void convolve_horizontal(int width, int height, float* input, float* output, int kernel_size, float* kernel) {
    const int thisX = threadIdx.x + blockIdx.x * blockDim.x;
    const int thisY = threadIdx.y + blockIdx.y * blockDim.y;
    ...
    int center = kernel_size / 2, dx, tx, ty;
...
Image gaussian_blur(const Image& img, float sigma)
{
    ...
    cudaMemcpy(d_kernel, kernel.data, size * sizeof(float), cudaMemcpyHostToDevice);

    float* d_img;
    cudaMalloc(&d_img, width * height * sizeof(float));
    cudaMemcpy(d_img, img.data, width * height * sizeof(float), cudaMemcpyHostToDevice);

    float *d_output1, *d_output2;
    cudaMalloc(&d_output1, width * height * sizeof(float));
    cudaMalloc(&d_output2, width * height * sizeof(float));

    dim3 threadsPerBlock(32, 32);
    dim3 numBlocks((width + threadsPerBlock.x - 1) / threadsPerBlock.x, (height + threadsPerBlock.y - 1) / threadsPerBlock.y);
    convolve_vertical<<<numBlocks, threadsPerBlock>>>(width, height, d_img, d_output1, size, d_kernel);
    cudaDeviceSynchronize();

    Image tmp(img.width, img.height, 1);
    Image filtered(img.width, img.height, 1);

    convolve_horizontal<<<numBlocks, threadsPerBlock>>>(width, height, d_output1, d_output2, size, d_kernel);
    cudaDeviceSynchronize();

    cudaMemcpy(filtered.data, d_output2, width * height * sizeof(float), cudaMemcpyDeviceToHost);
