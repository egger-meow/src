...
{
    float x_i, y_j;
    // #pragma omp parallel for private(x_i, y_j) collapse(2)
                                                     ...
                                                     float lambda_ori, float lambda_desc)
{
    ...
    printf("compute_keypoint_orientations&descriptor: = %.3g secs\n", end-start);
    return kps;
}
